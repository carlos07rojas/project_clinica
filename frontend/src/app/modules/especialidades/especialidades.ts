...
export class Especialidades implements OnInit {
  ...
  cargarEspecialidades(): void {
    ...
    this.especialidadService.obtenerTodas().subscribe({
      // subscribe() espera la respuesta del backend
      // next = se ejecuta cuando el backend responde bien
      next: (data) => {
        // data es el array de especialidades que devuelve el backend
  ...
  crearEspecialidad(): void {
    ...
    if (!this.nuevaEspecialidad.nombre.trim()) {
      //trim. este componente elimina espacios en blanco al inicio y al final
    ...
    this.especialidadService.crear(this.nuevaEspecialidad).subscribe({
      next: (data) => {
        // data es la especialidad recién creada que devuelve el backend
